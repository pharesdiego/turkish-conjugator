const { map, reduce, filter } = require('./utils');
...
const strEndsWith = (string) => (...substrs) =>
  substrs.some((substr) => string.endsWith(substr));

const strStarsWith = (string) => (...substrs) =>
  substrs.some((substr) => string.startsWith(substr));
