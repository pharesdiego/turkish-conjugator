...
const { alphabet } = require('./../rules');

const hasMinLength = (str, l) => str.length >= l;

const isVerb = (verb) => strEndsWith(verb.toLowerCase())('mek', 'mak');

const isNegativeVerb = (verb) =>
  strEndsWith(verb.toLowerCase())('memek', 'mamak');

const convertToPositive = (verb) =>
  verb.toLowerCase().replace(/\w{5}$/, (m) => m[0] + m[1] + 'k');

const convertToNegative = (verb) =>
  verb.toLowerCase().replace(/\w{3}$/, (m) => m[0] + m[1] + m[0] + m[1] + 'k');
 ...
 */
const isAlphabeticallyValid = (str) =>
  str
    .toLowerCase()
    .split('')
...
const getVowelsFromInitOfComposedVerb = _(
  splitSpace,
  arrInit,
  join,
  getVowelsStr
