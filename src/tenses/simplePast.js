const { push, lastLetter } = require('./../methods/basics');
...
const SimplePast = (verb, DEFAULT = getProperties(verb)) => {
  ...
  let root = mutation[0].from.includes(lastLetter(DEFAULT.originalRoot))
    ? DEFAULT.originalRoot + 't'
    : DEFAULT.root + 'd';
  ...
  let personalSuffixes = push(
    arrayOfPersonalSuffixes.II(DEFAULT.harmony4way),
    larOrLer
  );
