const { push, strInit, isVowel, lastLetter } = require('./../methods/basics');
...
const PotentialPositive = (verb, DEFAULT = getProperties(verb)) => {
  ...
  let root =
    lastLetter(DEFAULT.root) === 'e' && DEFAULT.vowelsLength === 1
      ? strInit(DEFAULT.root) + 'i'
      : DEFAULT.root;
  ...
  let abilitySuffix =
    (isVowel(lastLetter(root)) ? 'y' : '') + DEFAULT.harmony2way + 'bil';
